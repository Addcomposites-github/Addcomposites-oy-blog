...
/* ===== flaperon infographics ===== */

/* ---------- bflow · build-route comparison ---------- */
(function () {
  var root = document.getElementById("bflow-root");
  if (!root) return;
  var obs = new IntersectionObserver(
    function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          root.classList.add("bflow-animated");
          obs.unobserve(root);
        }
      });
    },
...
/* ---------- cmap · complexity vs process maturity scatter ---------- */
(function () {
  var root = document.getElementById("cmap-root");
  if (!root) return;
  root.querySelectorAll(".cmap-point").forEach(function (pt) {
    var x = pt.getAttribute("data-x");
    var y = pt.getAttribute("data-y");
    if (x !== null) pt.style.setProperty("--x", x);
    if (y !== null) pt.style.setProperty("--y", y);
  });
  var obs = new IntersectionObserver(
    function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          root.classList.add("cmap-animated");
          obs.unobserve(root);
        }
      });
    },
